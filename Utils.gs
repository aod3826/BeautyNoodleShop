...
function logAction(action, details, userId) {
  ...
  } catch (error) { console.error('Log failed:', error); console.log({action,details,userId,error:error.message}); }
}
...
function createBackup() {
  try {
    ...
    if (!folderIterator.hasNext()) backupFolder = DriveApp.createFolder('BeautyNoodleBackups');
...
function systemHealthCheck() {
  try {
    ...
    } catch (e) { health.checks.spreadsheet = { status: 'error', message: e.message }; health.status = 'degraded'; }
    const lineConfig = getLineConfig();
    health.checks.line = { status: lineConfig.channelAccessToken ? 'ok' : 'warning', configured: isLineMessagingReady(), liffConfigured: !!lineConfig.liffId, message: lineConfig.channelAccessToken ? 'LINE configured' : 'LINE not fully configured' };
    try {
      const cache = CacheService.getScriptCache();
      cache.put('health_check', 'ok', 60);
      health.checks.cache = { status: 'ok' };
    } catch (e) { health.checks.cache = { status: 'error', message: e.message }; }
    try {
      const ss = getSpreadsheet();
      const logsSheet = ss.getSheetByName('Logs');
      if (logsSheet) { const logCount = logsSheet.getLastRow() - 1; health.checks.logs = { status: logCount<9000?'ok':'warning', count: logCount, message: logCount<9000?'Logs within limit':'Logs approaching limit' }; }
    } catch (e) { health.checks.logs = { status: 'error', message: e.message }; }
    return { success: true, data: health };
  } catch (error) { logAction('HEALTH_CHECK_ERROR', error.message, 'SYSTEM'); return { success: false, error: error.message }; }
}
